        ...
        <div class="row petss">
            @foreach($petData as $pet)
                <div class="col-2 border p-4 info bg-white ms-2 my-2 rounded" >
                    <div>
                        <span><b>Name</b> : {{ $pet->petName }} </span><br>
                        <span><b>Age</b> : {{ $pet->petAge }} </span><br>
                        <span><b>Gender</b> : {{ $pet->petGender }} </span><br>
                        <span><b>Type</b> : {{ $pet->petBreed }} </span><br>
                        <span><b>Status</b> : {{ $pet->petStatus }} </span><br>
                    </div>
                    <div class="mt-3">
                        <form action="{{ Route('delete.pets', ['id' => $pet->id ]) }}">
                            @csrf
                            @method('DELETE')
                            <button class="del-btn btn btn-danger btn-sm">Delete Pet</button>
                        </form>
                        <form action="{{ Route('edit.pet', ['id' => $pet->id ]) }}">
                            @csrf
